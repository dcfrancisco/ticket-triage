...
A structured, weekend-friendly roadmap to build a Spring Boot (Java 21) backend for AI-assisted ticket triage. Includes pgvector-powered similarity (feature-flagged) and AI adapters for stub, OpenAI, or local HTTP LLMs (e.g., Ollama/vLLM). This repo currently contains learning guides, macOS setup notes, and scripts to export docs to PDF.
...
- `docs/stretch-weekend.md` — optional similarity (pgvector), compose-to-k8s, cloud deployment options, CI.
...
- `scripts/export-docs.sh` — render PDFs via pandoc (PDFs are git-ignored).
...
5) Export docs to PDF (optional)
   ...
   - PDFs land in `build/pdfs` (ignored by git).
